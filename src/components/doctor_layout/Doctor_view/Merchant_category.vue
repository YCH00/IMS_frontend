  ...
  <div>
    ...
    <div class="action-bar">
      ...
      <div class="search-group">
        ...
        <Button label="新建品类" icon="pi pi-plus" @click="openCreateDialog"
                class="p-button-success new-category-button"/>
        <Button as="router-link" label="返回餐馆页" icon="pi pi-arrow-left" to="/merchant_home/Merchant_restaurant"
                severity="help"/>
      </div>
    </div>
    ...
    <DataTable :value="filteredCategory" datakey="id" sortField="sort" :sortOrder="-1" responsiveLayout="scroll"
      ...
      <Column header="操作">
        <template #body="slotProps">
          <!--修改按钮-->
          <Button icon="pi pi-pencil" severity="success" class="operation-button"
                  @click="openEditDialog(slotProps.data)"/>
          <!--删除按钮-->
          <Button icon="pi pi-trash" severity="danger" class="operation-button"
      ...
      <template #expansion="slotProps">
        <div class="p-4">
          <h5>菜品列表</h5>
          <DataTable :value="slotProps.data.dishes" datakey="id" sortField="sort" :sortOrder="-1"
    ...
    <Dialog header="创建/编辑菜品" v-model:visible="visible">
      ...
      <template #footer>
        ...
        <Button label="保存" icon="pi pi-check" class="p-button-primary"
                @click="editingCategory ? updateCategory() : createCategory()"/>
      </template>
    </Dialog>
  </div>
</template>
