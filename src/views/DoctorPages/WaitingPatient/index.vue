<template>
    <div class="patient-waiting-management">
        ...
        <table>
            ...
            <tbody>
                <tr v-for="patient in paginatedPatients" :key="patient.id" class="patient-row"
                    @mouseover="hoveredRow = patient.id" @mouseleave="hoveredRow = null">
                    ...
                    <td>
                        <button class="action-btn call-btn hover-btn" @click="openCallDialog(patient)">
                            叫号
                        </button>
                        <button class="action-btn complete-btn hover-btn" @click="completePatient(patient.id)">
                            完成
                        </button>
                    </td>
                </tr>
        ...
        <el-pagination @current-change="handlePageChange" @size-change="handleSizeChange"
            :current-page="paginationData.pageNumber" :page-size="paginationData.pageSize" :total="patients.length"
        ...
        <el-dialog v-model="dialogVisible" width="70%" title="处方" class="prescription-dialog" :top="'10px'" >
            <div class="visit-interface">
                ...
                <div class="module " @mouseover="hover = true" @mouseleave="hover = false">
                    <h2>患者信息</h2>
                    <div class="flex-box">
                        <div class="flex-box-item">姓名：<span>{{ currentPatient.name }}</span></div>
                        <div class="flex-box-item">性别：<span>{{ currentPatient.gender }}</span></div>
                        <div class="flex-box-item">年龄：<span>{{ currentPatient.age }}</span></div>
                    </div>
                ...
                <div class="module" @mouseover="hover = true" @mouseleave="hover = false">
                    <h3>病历</h3>
                    <el-input type="textarea" v-model="medicalRecord" placeholder="填写病历..." rows="5"
                        class="input-field" />
                </div>
                ...
                <div class="module" @mouseover="hover = true" @mouseleave="hover = false">
                    ...
                    <table v-if="prescriptions.length > 0" class="prescription-table">
                        ...
                        <tbody>
                            <tr v-for="(prescription, index) in prescriptions" :key="index">
                                <td>{{ index + 1 }}</td>
                                <td>
                                    <el-select v-model="prescription.name" :placeholder="'选择药品'"
                                        @change="updatePrice(index)">
                                        <el-option v-for="(item, index) in drugList" :key="item.name" :label="item.name"
                                            :value="item.name" />
                                    </el-select>
                                </td>
                                ...
                                <td>
                                    <el-button @click="removePrescription(index)" size="small"
                    ...
                    <!-- 始终显示“添加药品”按钮 -->
                    <el-button v-show="true" @click="addPrescription" class="add-prescription-btn"
                        size="small">添加药品</el-button>
                </div>

                <!-- 医生建议模块 -->
                <div class="module" @mouseover="hover = true" @mouseleave="hover = false">
                    <h3>医生建议</h3>
                    <el-input type="textarea" v-model="doctorAdvice" placeholder="填写医生建议..." rows="5"
                        class="input-field" />
                </div>

                <!-- 保存按钮 -->
                <div class="actions">
                    <el-button type="primary" @click="savePrescription" class="save-btn" round icon="el-icon-check">
                        保存
                    </el-button>
                </div>
            </div>
