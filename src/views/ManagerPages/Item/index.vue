  ...
  <div class="admin-tests-management">
    ...
    <table>
      ...
      <tr v-for="test in tableData.list" :key="test.id" class="test-row" @mouseover="hoveredRow = test.id"
        ...
        <td>{{ test.address }}</td>
        <td>
          <el-button size="default" class="action-btn edit-btn" @click="openEditDialog(test)">
            编辑
          </el-button>
          <el-button size="default" type="danger" class="action-btn delete-btn" @click="deleteTest(test.id)">
            删除
          </el-button>
        </td>
      </tr>
