  ...
  <div class="appointment-management">
    ...
    <table>
      ...
      <tr v-for="appointment in tableData.list" :key="appointment.id" class="appointment-row"
        ...
        <td>{{ appointment.doctor.dept_name }}</td>
        <td>{{ appointment.doctor.name }}</td>
        <td>{{ appointment.appointment_date }}</td>
        <td>{{ appointment.appointment_time }}</td>
        <td>
          <el-button size="default" class="action-btn edit-btn" @click="editAppointment(appointment)">
            编辑
          </el-button>
          <el-button size="default" type="danger" class="action-btn delete-btn"
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑预约' : '新增预约'" class="appointment-dialog" width="50%">
      <el-form :model="currentAppointment" label-width="120px" ref="appointmentForm" :rules="rules"
        ...
        <el-form-item label="患者姓名">
          <el-input v-model="currentAppointment.user.name" :disabled=true placeholder="请输入患者姓名"/>
        </el-form-item>
        <el-form-item label="科室">
          <el-input v-model="currentAppointment.doctor.dept_name" :disabled=true placeholder="请输入科室"/>
        </el-form-item>
        <el-form-item label="医生姓名">
          <el-select v-model="currentAppointment.doctor.id" placeholder="请选择医生" style="width: 100%">
...
import {getAllAppoint, alterAppoint, delAppoint, addAppoint, getDoctorsByDept} from "../../../api/index.js"; // 这里需要你的接口
...
const onUpdateAppointment = async () => {
  ...
  if (currentAppointment.value) {
    try {
      const {data} = await getDoctorsByDept(currentAppointment.value.doctor.dept_id);
      doctors.value = data.data.list;
    } catch (error) {
...
const saveAppointment = () => {
  appointmentForm.value.validate((valid) => {
    if (valid) {
      ...
      const date = currentAppointment.value.appointment_date;
      const time = currentAppointment.value.appointment_time;
      console.log(currentAppointment.value.time, "time")
      const params = {
        user_id: currentAppointment.value.user.id,
        doctor_id: currentAppointment.value.doctor.id,
        appointment_time: currentAppointment.value.time ? currentAppointment.value.time : `${date}T${time}`
      };
      if (isEdit.value) { // 修改
        alterAppoint(params, id).then((res) => {
