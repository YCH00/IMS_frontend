  ...
  <div class="admin-drug-management">
    ...
    <table>
      ...
      <tr v-for="drug in tableData.list" :key="drug.id" class="drug-row" @mouseover="hoveredRow = drug.id"
        ...
        <td>{{ drug.number }}</td>
        <td>
          <el-button size="default" class="action-btn edit-btn" @click="openEditDialog(drug)">
            编辑
          </el-button>
          <el-button size="default" type="danger" class="action-btn delete-btn" @click="deleteDrug(drug.id)">
            删除
          </el-button>
        </td>
      </tr>
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑药品' : '新增药品'" class="drug-dialog" width="50%">
      <el-form :model="currentDrug" label-width="80px" ref="drugForm" :rules="rules" @submit.native.prevent>
        ...
        <el-form-item label="药品描述" prop="num">
          <el-input v-model="currentDrug.description" type="textarea" placeholder="请输入药品描述" :rows="5"
                    resize="none"/>
        </el-form-item>
      </el-form>
