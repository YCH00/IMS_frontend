<template>
  <div class="registration-page">
    <div class="form-card" @mouseenter="cardHover = true" @mouseleave="cardHover = false" :class="{ hover: cardHover }">
      ...
      <!-- 科室选择 -->
      <div class="form-group" @mouseenter="hoverDepartment = true" @mouseleave="hoverDepartment = false"
           :class="{ hover: hoverDepartment }">
        <label for="department">
          <i class="icon-department"></i> 科室
        </label>
        <select id="department" v-model="selectedDepartment" class="input-field" @change="onDepartmentChange">
      ...
      <!-- 医生选择 -->
      <div class="form-group" @mouseenter="hoverDoctor = true" @mouseleave="hoverDoctor = false"
           :class="{ hover: hoverDoctor }">
        <label for="doctor">
      ...
      <!-- 预约日期 -->
      <div class="form-group" @mouseenter="hoverDate = true" @mouseleave="hoverDate = false"
           :class="{ hover: hoverDate }">
        <label for="date">
      ...
      <!-- 预约时间 -->
      <div class="form-group" @mouseenter="hoverTime = true" @mouseleave="hoverTime = false"
           :class="{ hover: hoverTime }">
        <label for="time">
...
import {getAllDept, getDoctorsByDept, addAppoint} from "../../../api/index.js";
...
const onDepartmentChange = async () => {
  ...
  if (selectedDepartment.value) {
    try {
      const {data} = await getDoctorsByDept(selectedDepartment.value); // 请求医生数据
      doctors.value = data.data.list; // 更新医生列表
    } catch (error) {
      console.error("加载医生数据失败:", error);
    }
  } else {
    doctors.value = []; // 如果未选择科室，清空医生列表
  }
};
...
const handleSubmit = () => {
  if (selectedDepartment.value && selectedDoctor.value && selectedDate.value && selectedTime.value) {
    ...
    addAppoint({
      ...
      appointDateTime: `${selectedDate.value}T${selectedTime.value}`
    });
...
.registration-page::after {
  ...
  right: 0;
  bottom: 0;
  background: linear-gradient(45deg, rgba(255, 255, 255, 0.2) 25%, transparent 25%) -5px -5px,
